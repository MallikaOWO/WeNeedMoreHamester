...
双 iframe 架构，通过 `initializeGlobal`/`waitGlobalInitialized` 桥接：

- **Tavern 脚本** `src/鼠鼠天堂/脚本/自动更新/index.ts` — 运行在脚本 iframe，调用 SillyTavern API（`importRawCharacter`、`replaceCharacter`、世界书备份、`toastr`），通过 `initializeGlobal('UpdateService', ...)` 暴露接口
- **React 前端** `src/鼠鼠天堂/界面/主界面/update.ts` + `update-check.ts` + `views/Settings.tsx` — 运行在消息 iframe，负责 manifest 获取、版本比较、渲染更新 UI，通过轮询 `window.top.UpdateService` 调用脚本接口

URL 方案：
- manifest.json 从 `raw.githubusercontent.com` 获取（文本文件，非 LFS，避免 CDN 缓存）
- 角色卡 PNG 从 `media.githubusercontent.com` 下载（支持 Git LFS 大文件；`raw.githubusercontent.com` 和 jsdelivr 都只返回 LFS 指针）

`manifest.json` 位于仓库根目录，发行文件位于 `release_src/发行角色卡/`。
...
- **角色定义之前**（一般条目）：顺序越大，在最终提示词里位置越靠后
- **指定深度**：深度越深，位置越靠近聊天记录顶部（深度 0 = 聊天记录最下方）
- 同一深度内，顺序越大越靠后
- 使用简单数字 1/2/3，不要用大数

当前排列：
- 角色定义之前：天使人设 = 1，角色档案 = 2
- 指定深度 0：状态摘要 = 1，事件指令 = 2，变量列表 = 3，变量更新规则 = 4，变量输出格式 = 5
