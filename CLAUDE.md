...
- **React 前端** `src/鼠鼠天堂/界面/主界面/update.ts` + `update-check.ts` + `views/Settings.tsx` — 运行在消息 iframe，负责 manifest 获取、版本比较、渲染更新 UI，通过 `waitGlobalInitialized('UpdateService')` 调用脚本接口

URL 方案：
- manifest.json 从 `raw.githubusercontent.com` 获取（避免 CDN 缓存）
- 角色卡文件从 `cdn.jsdelivr.net` 下载（内容不变，用 CDN 加速）
