...
export function canBuild(state: GameState, facilityType: string): { ok: boolean; reason?: string } {
  ...
  if (def.manageDomain !== 'any') {
    const angel = state.angels.find(a => a.manageDomain === def.manageDomain);
...
export function buildFacility(state: GameState, facilityType: string): GameState {
  ...
  if (def.manageDomain !== 'any') {
    const angel = state.angels.find(a =>
      a.manageDomain === def.manageDomain && !a.assignedFacility
    );
    if (angel) managedBy = angel.id;
  }
  ...
  const angels = managedBy
    ? state.angels.map(a => a.id === managedBy ? { ...a, assignedFacility: id } : a)
    : state.angels;
...
export function canUpgrade(state: GameState, facilityId: string): { ok: boolean; reason?: string } {
  ...
  if (facility.managedBy) {
    const angel = state.angels.find(a => a.id === facility.managedBy);
    ...
    facilities: state.facilities.map(f =>
      f.id === facilityId
        ? {
            ...
            level: f.level + 1,
            // 升级容量：每级 +1（有容量的设施）
            capacity: def.capacity > 0 ? def.capacity + f.level : 0,
          }
        : f
    ),
  };
...
export function assignAngelToFacility(state: GameState, angelId: string, facilityId: string): GameState {
  ...
  const facilities = state.facilities.map(f => {
    if (f.id === facilityId) return { ...f, managedBy: angelId };
    if (f.managedBy === angelId) return { ...f, managedBy: null };
    return f;
  });

  const angels = state.angels.map(a =>
    a.id === angelId ? { ...a, assignedFacility: facilityId } : a
  );
